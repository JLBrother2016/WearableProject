...
#if _FS_LOCK
static FILESEM	Files[_FS_LOCK];	/* File lock semaphores */
									//文件锁信号量
#endif
...
/* vol: Logical drive number to be mounted/unmounted, 将要挂载/卸载的逻辑驱动号 */
/* fs: Pointer to new file system object (NULL for unmount)*/
/*
	此函数的作用是在磁盘里注册一个缓冲区域，用来存储FAT32文件系统的一些相关信息，
对磁盘经行操作之前，这个函数时不可少的。该函数只是初始化给定的工作区和注册内部表
的地址，并没有进行媒体访问。
*/
/*-----------------------------------------------------------------------*/
FRESULT f_mount(BYTE vol, FATFS *fs)
{
	...
	if (vol >= _VOLUMES)		/* Check if the drive number is valid, vol = SD = 0 */
		return FR_INVALID_DRIVE;
	rfs = FatFs[vol];			/* Get current fs object, FatFs[vol]是静态全局全局变量 */

	if (rfs)
	{
#if _FS_LOCK
		clear_lock(rfs);		/* Clear lock entries of the volume*/
#endif
...
/* 
	...
	函数成功时会创建一个文件对象，以供随后的读写函数调用文件时使用，使用f_close
函数来关闭一个打开的文件对象，如果修改的文件没有被关闭，文件数据将会丢失，在使用
任何文件函数之前，必须对相应的逻辑驱动器使用f_mount函数注册一个工作区（文件系统对象）
所有的文件函数在注册工作区后才能正常工作。
*/
/*-----------------------------------------------------------------------*/
...
FRESULT f_open(FIL *fp, const TCHAR *path, BYTE mode)
{
	...
	DIR dj;
	BYTE *dir;
	DEF_NAMEBUF;			/* BYTE sfn[12]; WCHAR lbuf[_MAX_LFN+1] */
...
/* 
	...
	每次f_read函数执行完后，*br值等于本次读取的字节数，如果*br < btr，则表示读写
指针在读操作期间已经到达文件末尾
*/
/*-----------------------------------------------------------------------*/
...
/* 
	...
	每次f_write函数执行完后， *bw 值等于本次写入的字节数，如果*bw < btw,则
表示写操作期间卷已满。
*/
/*-----------------------------------------------------------------------*/
...
/*
	改变驱动器的当前目录
	path 	指向要跳转到的空结尾字符串目录的指针
	f_chdir函数改变驱动器的当前目录，当驱动器自动注册的时候，当前目录初始化为
根目录；在每个文件系统对象中，当前目录是保持的，所以这会影响到使用设个驱动器的其他任务
*/
...
/*
	检索当前目录
	buff 		指向接收当前目录字符串缓冲区的指针
	len			缓冲区大啊啊啊小
	f_getcwd函数会将当前目录的绝对路径复制到缓冲区
*/
#if _FS_RPATH >= 2
...
/*
	...
	偏移值可以在文件起始和结尾范围内指定，当在写模式下偏移值超出文件大小后，文件
大小会增加，扩充区域的数据是未定义的。这适合快速创建大文件，尤其是快速的写操作，
f_lseek函数成功后，应该对文件读写指针fptr检查以确保读写指针被正确移动，如果fptr
不是期望值，可能是发生了一下情况：
	文件结束，指定的偏移值被文件大小截断是因为开启了只读模式
	磁盘已满，卷上没有足够的空间扩展文件大小。
*/
/*-----------------------------------------------------------------------*/
...
/*
	...
	fno				Pointer to file information to return
	f_sta函数获取一个文件或目录的信息，如最近修改时间，属性，获取的信息存在
FileInfo结构体中；如果目标是文件夹，获取的大小为0，此函数对根目录无效，对于
信息的细节，参考FILINFO结构体和f_readdir函数
*/
/*-----------------------------------------------------------------------*/
...
#if !_FS_READONLY
/*-----------------------------------------------------------------------*/
/* Get Number of Free Clusters 获取空闲簇*/
/*
	path			Path name of the logical drive number
	nclst			Pointer to a variable to return number of free clusters
	fatfs			Pointer to return pointer to corresponding file system object
	f_getfree函数获取驱动器上的空闲簇，文件系统对象的成员csize代表每簇的扇区数，所以
剩余空间可以由单位扇区计算出来，当FSInfo结构在FAT32卷上不同步时，函数会返回一个错误
的空闲簇
*/
/*-----------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------*/
/* Create a Directory 创建一个新目录   */
/*
	path			Pointer to the directory path
	f_mkdir函数创建一个新目录，目录名应该符合FatFs标准，不能包含非法字符，不能
用来创建文件，不能在不存在的目录下创建新目录；若不支持长文件名，文件名长度不能
大于8，否则创建不成功、
*/
/*-----------------------------------------------------------------------*/
...
#if _USE_STRFUNC
/*-----------------------------------------------------------------------*/
/* Get a string from the file 从文件中读取字符串                */
/*
	buff			Pointer to the string buffer to read
	len				Size of string buffer (characters)
	fp				Pointer to the file object
	f_gets是f_read的封装函数，读操作持续指导存储一个'\n'字符，到达文件末尾，或者
缓冲区已经填充了size-1个字符；
	读字符串以'\0'结尾，当没有字符可读或者在读操作期间出现任务错误，f_gets会返回
一个空指针。文件结束和错误状态可以通过调用f_eof和f_error得知。
*/
/*-----------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------*/
/* Put a formatted string to the file 往文件中写入格式化字符串*/
/* 
	fp 			Pointer to the file object
