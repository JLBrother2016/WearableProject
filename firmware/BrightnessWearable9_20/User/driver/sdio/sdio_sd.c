 ...
 * 与SD卡相关的返回出错码解释
*/
static void sdio_err_info(__IO SD_Error errcode)
{
	switch (errcode) {
		case 1:
			debug("Command response received (but CRC check failed)\r\n");
			break;
		case 2:
			debug("Data bock sent/received (CRC check Failed)\r\n");
			break;
		case 3:
			debug("Command response timeout\r\n");
			break;
		case 4:
			debug("Data time out\r\n");
			break;
		case 5:
			debug("Transmit FIFO under-run\r\n");
			break;
		case 6:
			debug("Receive FIFO over-run\r\n");
			break;
		case 7:
			debug("Start bit not detected on all data signals in widE bus mode\r\n");
			break;
		case 8:
			debug("CMD's argument was out of range\r\n");
			break;
		case 9:
			debug("Misaligned address\r\n");
			break;
		case 10:
			debug("Transferred block length is not allowed for the card or the number of transferred bytes does not match the block length\r\n");
			break;
		default:
			debug("unkonwn error code\r\n");
	}
}
...
SD_Error SD_Init(void)
{
  ...
  errorstatus = SD_PowerON();		/* 确保SD卡的电压和配置时钟 */
  if (errorstatus != SD_OK)
  {
    /*!< CMD Response TimeOut (wait for CMDSENT flag) */
		debug("SD_PowerON error! errorstatus: %d\r\n", errorstatus);
		sdio_err_info(errorstatus);
		return(errorstatus);
  }
  ...
  {
    /*!< CMD Response TimeOut (wait for CMDSENT flag) */
		debug("SD_InitializeCards fialed! errorstatus: %d\r\n", errorstatus);
		sdio_err_info(errorstatus);
		return(errorstatus);
  }
  ...
  if (errorstatus != SD_OK) {																/* + */
	  err("SD_GetCardInfo failed!, errorstatus: %d\r\n", errorstatus);						/* + */
	  sdio_err_info(errorstatus);															/* + */
  }																							/* + */
  ...
  if (errorstatus == SD_OK)
  {
    /*----------------- Select Card --------------------------------*/
		...
		if (errorstatus != SD_OK) {															/* + */
			err("SD_SelectDeselect failed!, errorstatus: %d\r\n", errorstatus);				/* + */
			sdio_err_info(errorstatus);														/* + */
	  }																						/* + */
  }
  ...
  if (errorstatus == SD_OK)
  {
		errorstatus = SD_EnableWideBusOperation(SDIO_BusWide_4b);							/* 这边有问题，返回错误代码14 */
		if (errorstatus != SD_OK) {															/* + */
			err("SD_EnableWideBusOperation failed!, errorstatus: %d\r\n", errorstatus);		/* + */
			sdio_err_info(errorstatus);														/* + */
		}																					/* + */
  }
...
SD_Error SD_EnableWideBusOperation(uint32_t WideMode)
{
  ...
  if (SDIO_MULTIMEDIA_CARD == CardType)
  {
		...
		err("SDIO_MULTIMEDIA_CARD: SD_UNSUPPORTED_FEATURE\r\n");
		return(errorstatus);
  }
  else if ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == CardType) || (SDIO_STD_CAPACITY_SD_CARD_V2_0 == CardType) || (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
  {
    ...
    {
		errorstatus = SD_UNSUPPORTED_FEATURE;
		err("SDIO_BusWide_8b: SD_UNSUPPORTED_FEATURE\r\n");
		return(errorstatus);
    }
    else if (SDIO_BusWide_4b == WideMode)
    {
      ...
      if (SD_OK == errorstatus)
      {
        ...
        SDIO_Init(&SDIO_InitStructure);
			...
			err("SDEnWideBus(ENABLE) failed!, errorstatus: %d\r\n", errorstatus);
	  }
    }
    else
    {
      ...
      if (SD_OK == errorstatus)
      {
        ...
        SDIO_Init(&SDIO_InitStructure);
			...
			err("SDEnWideBus(DISABLE) failed!, errorstatus: %d\r\n", errorstatus);
	  }
    }
  }
...
{
	if (SDIO_GetITStatus(SDIO_IT_DATAEND) != RESET)	{			/* 接受完成中断 */
		TransferEnd = 1;
		SDIO_ClearITPendingBit(SDIO_IT_DATAEND);
		TransferError = SD_OK;
	}
	else if (SDIO_GetITStatus(SDIO_IT_DCRCFAIL) != RESET) { 	/* 数据CRC错误 */
		SDIO_ClearITPendingBit(SDIO_IT_DCRCFAIL);
		TransferError = SD_DATA_CRC_FAIL;
	}
	else if (SDIO_GetITStatus(SDIO_IT_DTIMEOUT) != RESET) {		/* 数据超时错误 */
		SDIO_ClearITPendingBit(SDIO_IT_DTIMEOUT);
		TransferError = SD_DATA_TIMEOUT;
	}
	else if (SDIO_GetITStatus(SDIO_IT_RXOVERR) != RESET) {		/* FIFO 上溢错误 */
		SDIO_ClearITPendingBit(SDIO_IT_RXOVERR);
		TransferError = SD_RX_OVERRUN;
	}
	else if (SDIO_GetITStatus(SDIO_IT_TXUNDERR) != RESET)		/* FIFO下溢错误 */
	{
		SDIO_ClearITPendingBit(SDIO_IT_TXUNDERR);
		TransferError = SD_TX_UNDERRUN;
	}
	else if (SDIO_GetITStatus(SDIO_IT_STBITERR) != RESET)		/* 起始位错误 */
	{
...
static SD_Error SDEnWideBus(FunctionalState NewState)
{
  ...
  if (errorstatus != SD_OK)
  {
	err("FindSCR failed! errorstatus: %d\r\n", errorstatus);
    return(errorstatus);
  }
  ...
  if (NewState == ENABLE)
  {
    ...
    if ((scr[1] & SD_WIDE_BUS_SUPPORT) != SD_ALLZERO)
    {
      ...
      if (errorstatus != SD_OK)
      {
			err("CmdResp1Error failed!, errorstatus: %d", errorstatus);
			return(errorstatus);
      }
      ...
      if (errorstatus != SD_OK)
      {
			err("CmdResp1Error failed!, errorstatus: \r\n", errorstatus);
			return(errorstatus);
      }
      return(errorstatus);
    }
...
//  SDDMA_InitStructure.DMA_BufferSize 						= 512;
...
void SDIO_Interrupts_Config(void)
{
	...
	NVIC_InitStructure.NVIC_IRQChannel = SDIO_IRQn;				/* Channel_49 */
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;	
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Init(&NVIC_InitStructure);								/* 抢占优先级0， 子优先级0 */

#if 1
	NVIC_InitStructure.NVIC_IRQChannel = SD_SDIO_DMA_IRQn;		/* DMA2_Stream3_IRQn Channel_59 */
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;	/* 抢占优先级0， 子优先级1  */
	NVIC_Init(&NVIC_InitStructure);
#endif
}
